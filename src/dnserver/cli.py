...
def handle_sig(signum, frame):  # pragma: no cover
    logger.info(
        "pid=%d, got signal: %s, stopping...", os.getpid(), signal.Signals(signum).name
    )
...
def cli_logic(args: list[str]) -> int:
    ...
    parser.add_argument(
        "--port",
        help="Port to run on, if omitted will use DNSERVER_PORT env var, or 53",
    )
    parser.add_argument(
        "--upstream",
        help=(
    ...
    if no_upstream:
        ...
        upstream = parsed_args.upstream or os.getenv(
            "DNSERVER_UPSTREAM", DEFAULT_UPSTREAM
        )
    zones_file = parsed_args.zones_file or os.getenv("DNSERVER_ZONE_FILE", None)
    if zones_file is None:
        print(
            "no zones file specified, use --help for more information", file=sys.stderr
        )
        return 1
