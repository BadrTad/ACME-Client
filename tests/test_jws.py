...
@pytest.fixture
def kid_header() -> Json:
    return {"url": "https://some_utl.com/1234", "nonce": "1234", "kid": "account_id_1234"}
...
def test_jws_factory_build_JWS_with_kid(crypto_keys, kid_header: Json, jws_payload: Json):
...
def test_jws_factory_build_JWS_with_kid_with_empty_payload(crypto_keys, kid_header: Json):
    jws_factory = JWSFactory(*crypto_keys)
...
def test_jws_factory_build_JWS_with_jwk(crypto_keys, jwk_header: Json, jws_payload: Json):
    jws_factory = JWSFactory(*crypto_keys)
...
def test_jws_factory_build_JWS_with_jwk_with_empty_payload(crypto_keys, jwk_header: Json):
    jws_factory = JWSFactory(*crypto_keys)
