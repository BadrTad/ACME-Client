...
def is_valid_signature(
    signature: str, deocoder: Callable[[str], bytes], signature_size_bytes=64
) -> bool:
...
def is_valid_kid(kid: str) -> bool:
    reg_kid_pattern = (
        r"^https:\/\/(\d+\.\d+\.\d+\.\d+):(\d+)\/my-account\/([0-9a-fA-F]+)$"
    )
    return bool(kid) and bool(re.match(reg_kid_pattern, kid))


def are_valid_identifiers(
    requested_identifiers: list[Identifier], received_identifiers: list[Identifier]
):
...
def is_valid_finalize(finalize: URL) -> bool:
    reg_finalize_pattern = (
        r"^https:\/\/(\d+\.\d+\.\d+\.\d+):(\d+)\/finalize-order\/([0-9a-zA-Z_-]+)$"
    )
...
def is_valid_order_url(order_url: URL) -> bool:
    reg_order_url_pattern = (
        r"^https:\/\/(\d+\.\d+\.\d+\.\d+):(\d+)\/my-order\/([0-9a-zA-Z_-]+)$"
    )
...
def are_valid_challenges(challenges: list[Challenge]) -> bool:
    def is_valid_challenge_url(url: URL) -> bool:
        reg_challenge_url_pattern = (
            r"^https:\/\/(\d+\.\d+\.\d+\.\d+):(\d+)\/chalZ\/([0-9a-zA-Z_-]+)$"
        )
    ...
    def is_valid_challenge(challenge: Challenge) -> bool:
        return (
            is_valid_challenge_type(challenge.type)
...
def dns_query(domain_name, rtype="TXT") -> Optional[list[str]]:
    ...
    try:
        ...
        response = resolver.query(
            domain_name, rtype, lifetime=5.0, raise_on_no_answer=True
        )
